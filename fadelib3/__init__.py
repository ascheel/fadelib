...
class FadeLIB(object):
	...
	def fill_solid(self,color):
		...
		if not isinstance(color, str):
			color = [list(color) for _ in range(self.NUM_LEDS)]
		else:
			color = [color for _ in range(self.NUM_LEDS)]
		return color
	...
	def fix_pixels(self,tmp_pixel=None):
		if tmp_pixel == None:
			tmp_pixel = copy.deepcopy(self.PIXELS)
		for pixel in range(len(tmp_pixel)):
	...
	def fix_colors(self,incolor):
		...
		outcolor = tuple(int(incolor[val] * self.BRIGHTNESS) for val in range(3))
	...
	def transition2(self, pixels2, **kwargs):
		...
		if not immediate:
			...
			for step in range(steps):
				percentage = step / steps
				for dot in range(self.NUM_LEDS):
					for color in range(3):
						self.PIXELS[dot] = tuple(int(pixels1[dot][color] + ((pixels2[dot][color] - pixels1[dot][color]) * percentage)) for color in range(3))
				if useglitter:
					self.add_glitter()
				if useconfetti:
					self.add_confetti()
			...
			self.PIXELS = self.fix_pixels(pixels2)
			self.update()
			self.update()
			time.sleep(duration)
	...
	def theme(self,season):
		...
		if season not in self.THEME:
			return None
		for color in range(self.NUM_LEDS):
